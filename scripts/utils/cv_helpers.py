...
def get_random_mask(inputs, height=260, width=346):
    # assert len(inputs.shape) == 3
    mask_left = np.random.randint(50, width-130)
    mask_right = mask_left + np.random.randint(10, 80)
    mask_top = np.random.randint(50, height-130)
    mask_bottom = mask_top + np.random.randint(10, 80)
    masked_input = copy.deepcopy(inputs)
    masked_input[..., mask_top:mask_bottom, mask_left:mask_right] = 0
    return masked_input