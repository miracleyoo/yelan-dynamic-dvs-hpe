...
from scripts.src.pose3d_utils.camera import CameraIntrinsics
from scripts.src.pose3d_utils.skeleton_normaliser import SkeletonNormaliser
...
class Skeleton:
    ...
    def proportion(self, torso_length):
        """ Get the proportion of actual torso length and input torso length.
        """
...
def prepare_label(label_raw, correct_intrinsic=False, width=346, height=260):
    # Parse raw label
    xyz = label_raw["xyz"].T
    intrinsic_matrix = torch.tensor(label_raw["camera"])
    extrinsic_matrix = torch.tensor(label_raw["M"])

    # Fix the width/height of camera intrinsic if wrong
    if correct_intrinsic:
        intrinsic_matrix[0,-2]=173
        intrinsic_matrix[1,-2]=130

    # Raw skeleton
    sk = Skeleton(xyz)

    # Project raw skeleton to camera plane
    ...
    # Normalize the projected skeleton based on its center distance
    sk_normalized = sk_onto_cam.normalize(height, width, intrinsic_matrix)
    sk_normalized = sk_normalized.get_masked_skeleton(mask)

    # 2D points of skeleton
