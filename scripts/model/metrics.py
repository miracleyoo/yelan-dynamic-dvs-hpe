...
class PCK(BaseMetric):
    ...
    """
    def __init__(self, reduction=None, threshold=0.15, from_auc=False, **kwargs):
        super().__init__(**kwargs)
        self.thr = threshold
        self.reduction = reduction
        self.from_auc = from_auc
...
class AUC(BaseMetric):
    ...
    at different thresholds (from 0 to 0.5)
    """
    def __init__(self,
                 reduction=None,
    ...
    def forward(self, y_pr, points_gt, gt_mask=None):
        pck_values = torch.DoubleTensor(len(self.thresholds)).to(y_pr)
        for i, threshold in enumerate(self.thresholds):
            _pck = PCK(self.reduction, threshold=threshold, from_auc=True)
            pck_values[i] = _pck(y_pr, points_gt, gt_mask)

        if self.auc_reduction is not None:
            pck_values = self.auc_reduction(pck_values)
